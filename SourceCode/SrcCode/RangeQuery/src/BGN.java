...
/*
 * This source code uses the JPBC (Java Pairing-Based Cryptography) library, 
 * which can be downloaded from http://gas.dia.unisa.it/projects/jpbc/
 */
...
public class BGN {
    /**
     * @ClassName: PublicKey
     * @Description: This is a class for storing the public key (n,G,GT,e,g,h)
     *               of BGN PKE.
     */
    public class PublicKey {
        ...
        public PublicKey(BigInteger n, Field<Element> G, Field<Element> GT,
     ...
     * @Description: This is a class for storing the private key (p) of BGN PKE.
     */
    ...
    /**
     * @Title: keyGeneration
     * @Description: This function is responsible for generating the public keys
     *               and the private keys.
     * @param k
     *            the security parameter, which decides the length of two large
     *            prime (p and q).
     * @return void
     */
    ...
    /**
     * @Title: getPubkey
     * @Description: This function returns the public key of BGN PKE.
    ...
    /**
     * @Title: getPrikey
     * @Description: This function returns the private key of BGN PKE.
     * @return PrivateKey The private key used to decrypt the data.
     */
    ...
    /**
     * @Title: encrypt
     * @Description: This function is to encrypt the message m, m in
     *               [0,1,2,...,T], T=100 with public key.
     * @param m
     *            The message
     * @param pubkey
     ...
     * @throws Exception
     *             If the plaintext is not in [0,1,2,...,n], there is an
     *             exception.
     */
    public static Element encrypt(int m, PublicKey pubkey) throws Exception {
        if (m > T) {
            throw new Exception(
                    "BGN.encrypt(int m, PublicKey pubkey): plaintext m is not in [0,1,2,...,"
                            + T + "]");
        }
     ...
     * 
     * @Title: decrypt
     * @Description: This function is to decrypt the ciphertext with the public
     ...
     *             exception.
     */
    public static int decrypt(Element c, PublicKey pubkey, PrivateKey prikey)
            throws Exception {
        ...
        throw new Exception(
                "BGN.decrypt(Element c, PublicKey pubkey, PrivateKey prikey): plaintext m is not in [0,1,2,...,"
                        + T + "]");
    }
    ...
    /**
     * @Title: add
     * @Description: The function supports the homomorphic addition with two
     *               ciphertext.
     * @param c1
     *            The ciphertext.
     * @param c2
    ...
    /**
     * @Title: mul1
     * @Description: The function supports the homomorphic multiplication with
     ...
     * @param pubkey
     *            The public key of BNG PKE.
     * @return Element The return value is e(c1,g) in GT. convToGT converts an Element in G into GT.
     */
    public static Element convToGT(Element c1, PublicKey pubkey) {
        Pairing pairing = pubkey.getPairing();
        return pairing.pairing(c1, pubkey.getG()).getImmutable();
    }
    
    /**
     * @Title: selfBlind
     * @Description: The function supports the homomorphic self-blinding with
     *               one ciphertext and one random number.
     * @param c
     *            The ciphertext.
     * @param r
    ...
    /**
     * @Title: getBGNInfo
     * @Description: The function is responsible for providing BGN instance information detail.
     * @param c
     *            The ciphertext.
     * @return BGN instance informaiton.
     */
    public String getBGNInfo(int k, PublicKey pubkey, PrivateKey prikey) {
    	String bgnInsInfo="";
    	bgnInsInfo += "BGN Key Generation (K="+k+") -»> \n";
    	bgnInsInfo += "  Public Key (n, G, GT, e, g, h)\n";
    	bgnInsInfo += "    n(pq) = " + pubkey.getN() + "\n";
    	bgnInsInfo += "    G = " + pubkey.getField_G() + "\n";
    	bgnInsInfo += "    GT = " + pubkey.getField_GT() + "\n";
    	bgnInsInfo += "    e(pairing) = " + pubkey.getPairing() + "\n";
    	bgnInsInfo += "    g = " + pubkey.getG() + "\n";
    	bgnInsInfo += "    h = " + pubkey.getH() + "\n";
    	bgnInsInfo += "  Private Key (p)\n";
    	bgnInsInfo += "    p = " + prikey.getP() + "\n";    	
    	return bgnInsInfo;
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (decrypted_c1expm2 == (m1 * m2)) {
                System.out
                        .println("Homomorphic multiplication-1 tests successfully.");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        try {
            Element c1 = BGN.encrypt(m1, pubkey);
            Element c2 = BGN.encrypt(m2, pubkey);
            Element c1pairingc2 = pubkey.getPairing().pairing(c1, c2)
                    .getImmutable();
            int decrypted_c1pairingc2 = BGN.decrypt_mul2(c1pairingc2, pubkey,
                    prikey);
            if (decrypted_c1pairingc2 == (m1 * m2)) {
                System.out
                        .println("Homomorphic multiplication-2 tests successfully.");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        m1 = 5;
        try {
            BigInteger r2 = pubkey.getPairing().getZr().newRandomElement()
                    .toBigInteger();
            Element c1 = BGN.encrypt(m1, pubkey);
            Element c1_selfblind = BGN.selfBlind(c1, r2, pubkey);
            int decrypted_c1_selfblind = BGN.decrypt(c1_selfblind, pubkey,
                    prikey);
            if (decrypted_c1_selfblind == m1) {
                System.out
                        .println("Homomorphic self-blinding tests successfully.");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
